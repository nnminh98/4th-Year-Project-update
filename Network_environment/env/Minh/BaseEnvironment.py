...
class BaseEnv(gym.Env):
    ...
    def convert_state_back(self, np_state):
        state = []
        for pkt_state in np_state:
            curr = self.graph.nodes["n{}".format(pkt_state[0])]
            src = self.graph.nodes["n{}".format(pkt_state[1])]
            dst = self.graph.nodes["n{}".format(pkt_state[2])]
            add_state = [curr, src, dst]
